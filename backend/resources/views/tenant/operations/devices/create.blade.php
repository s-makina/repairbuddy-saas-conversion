...
@section('content')
	<div class="container-fluid p-3">
		...
		<x-settings.card :title="__('Add Device')">
			<form method="post" action="{{ route('tenant.operations.devices.store', ['business' => $tenant->slug]) }}">
				@csrf
				<div class="row g-3">
					<div class="col-md-4">
						<x-settings.field for="model" :label="__('Model')" errorKey="model" class="wcrb-settings-field">
							<x-settings.input name="model" id="model" :value="old('model', '')" />
						</x-settings.field>
					</div>
					<div class="col-md-3">
						<x-settings.field for="device_type_id" :label="__('Type')" errorKey="device_type_id" class="wcrb-settings-field">
							<x-settings.select name="device_type_id" id="device_type_id" :options="$typeOptions" :value="old('device_type_id', '')" />
						</x-settings.field>
					</div>
					<div class="col-md-3">
						<x-settings.field for="device_brand_id" :label="__('Brand')" errorKey="device_brand_id" class="wcrb-settings-field">
							<x-settings.select name="device_brand_id" id="device_brand_id" :options="$brandOptions" :value="old('device_brand_id', '')" />
						</x-settings.field>
					</div>
					<div class="col-md-2">
						<x-settings.field for="parent_device_id" :label="__('Parent')" errorKey="parent_device_id" class="wcrb-settings-field">
							<x-settings.select name="parent_device_id" id="parent_device_id" :options="$parentOptions" :value="old('parent_device_id', '')" />
						</x-settings.field>
					</div>
					<div class="col-md-6">
						@php
							$disableChecked = (bool) old('disable_in_booking_form', false);
						@endphp
					...
					<div class="col-md-6">
						@php
							$otherChecked = (bool) old('is_other', false);
						@endphp
						<x-settings.option-toggle
							name="is_other"
							id="is_other"
