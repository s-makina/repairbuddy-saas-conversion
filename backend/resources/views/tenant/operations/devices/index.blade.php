@extends('tenant.layouts.myaccount', ['title' => $pageTitle ?? __('Devices')])
...
<script>
  (function () {
    ...
    var $table = window.jQuery('#devicesTable');
    if ($table.length === 0) {
      return;
    }
    ...
    $table.DataTable({
      ...
      serverSide: true,
      pageLength: 25,
      ajax: "{{ route('tenant.operations.devices.datatable', ['business' => $tenant->slug]) }}",
      order: [[1, 'asc']],
      columns: [
...
@section('content')
	<div class="container-fluid p-3">
		...
		<x-settings.card :title="__('Devices')">
			<div class="d-flex justify-content-end">
				<a class="btn btn-primary" href="{{ route('tenant.operations.devices.create', ['business' => $tenant->slug]) }}">{{ __('Add Device') }}</a>
			</div>

			<div class="mt-3 table-responsive">
				<table class="table table-sm align-middle mb-0" id="devicesTable">
