@extends('tenant.layouts.myaccount', ['title' => $pageTitle ?? __('Add Part Type')])
	...
	<div class="container-fluid p-3">
		...
		<div class="row justify-content-center g-10">
			<div class="col-12 col-lg-7 col-xl-8">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title mb-0">{{ __('Add Part Type') }}</h5>
					</div>
					<div class="card-body">
						<form method="post" action="{{ route('tenant.operations.part_types.store', ['business' => $tenant->slug]) }}" enctype="multipart/form-data">
							@csrf
							<div class="row g-3">
								<div class="col-12">
			...
			<div class="col-12 col-lg-5 col-xl-4">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title mb-0">{{ __('Recent part types') }}</h5>
					</div>
					<div class="card-body p-0">
						<div class="list-group list-group-flush">
							@forelse (($recentTypes ?? []) as $t)
								@php
									$editUrl = route('tenant.operations.part_types.edit', ['business' => $tenant->slug, 'type' => $t->id]);
								@endphp
								<div class="list-group-item d-flex align-items-center justify-content-between">
									<div class="text-truncate">
										<div class="fw-normal text-truncate">{{ (string) ($t->name ?? '') }}</div>
										@if (! empty($t->description))
											<div class="small text-muted text-truncate">{{ (string) $t->description }}</div>
										@endif
									</div>
							...
							@empty
								<div class="list-group-item">
									<div class="text-muted">{{ __('No part types yet.') }}</div>
								</div>
							@endforelse
						</div>
					</div>
