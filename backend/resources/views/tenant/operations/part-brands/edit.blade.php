@extends('tenant.layouts.myaccount', ['title' => $pageTitle ?? __('Edit Part Brand')])
			...
			<div class="col-12 col-lg-8 col-xl-6">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title mb-0">{{ __('Edit Part Brand') }}</h5>
					</div>
					<div class="card-body">
						<form method="post" action="{{ route('tenant.operations.part_brands.update', ['business' => $tenant->slug, 'brand' => $brand->id]) }}" enctype="multipart/form-data">
							@csrf
							<div class="row g-3">
								<div class="col-12">
										...
										<div class="col-sm-9">
											<select name="parent_id" id="parent_id" class="form-select @error('parent_id') is-invalid @enderror">
												...
												@foreach (($parentOptions ?? []) as $k => $v)
													<option value="{{ $k }}" @selected((string) old('parent_id', (string) ($brand->parent_id ?? '')) === (string) $k)>{{ $v }}</option>
												@endforeach
											</select>
											@error('parent_id')
												<div class="invalid-feedback">{{ $message }}</div>
									...
									<div class="row align-items-start">
										...
										<div class="col-sm-9">
											@if (! empty($brand->image_url))
												<div class="mb-2">
													<img src="{{ $brand->image_url }}" alt="{{ (string) ($brand->name ?? __('Part Brand')) }}" class="img-thumbnail" style="max-width: 160px; max-height: 160px; object-fit: contain;" />
												</div>
											@endif
