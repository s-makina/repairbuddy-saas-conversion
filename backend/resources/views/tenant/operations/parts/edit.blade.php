							...
							<div class="row g-3">
										...
										<div class="col-sm-9">
											<select name="tax_id" id="tax_id" class="form-select @error('tax_id') is-invalid @enderror">
												@foreach (($taxOptions ?? []) as $k => $v)
													<option value="{{ $k }}" @selected((string) old('tax_id', (string) ($part->tax_id ?? '')) === (string) $k)>{{ $v }}</option>
												@endforeach
											</select>
											@error('tax_id')
												<div class="invalid-feedback">{{ $message }}</div>
								...
								<div class="col-12">
									<div class="row align-items-start">
										...
										<div class="col-sm-9">
											<input type="number" name="stock" id="stock" class="form-control @error('stock') is-invalid @enderror" value="{{ old('stock', (string) ($part->stock ?? '')) }}" min="0">
											@error('stock')
												<div class="invalid-feedback">{{ $message }}</div>
											@enderror
										</div>
