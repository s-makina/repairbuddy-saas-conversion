...
<div class="sig-hero-header mb-4">
    ...
    <div class="d-flex gap-2 flex-shrink-0 flex-wrap">
        <a href="{{ route('tenant.estimates.signatures.index', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}" class="btn-hero-back">
            <i class="bi bi-list-ul me-1"></i>{{ __('All Requests') }}
        </a>
        <a href="{{ route('tenant.estimates.show', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}" class="btn-hero-back">
            <i class="bi bi-arrow-left me-1"></i>{{ __('Back to Estimate') }}
        </a>
    </div>
</div>
...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        ...
        @php
            $statusMap = ['pending'=>'secondary','approved'=>'success','rejected'=>'danger','converted'=>'info'];
            $statusColor = $statusMap[$estimate->status ?? ''] ?? 'secondary';
        @endphp
            ...
            <div class="card-body p-3">
                ...
                <div class="d-flex align-items-center gap-2 mb-3 pb-2 border-bottom">
                    <div class="customer-avatar-sm flex-shrink-0">
                        {{ strtoupper(substr($estimate->customer->name ?? 'C', 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-grow-1">
                        <div class="fw-semibold lh-1" style="font-size:.85rem;color:#111827;">{{ $estimate->customer->name ?? __('Unknown') }}</div>
                        @if(!empty($estimate->customer->email))
                        <div class="text-truncate text-muted" style="font-size:.73rem;" title="{{ $estimate->customer->email }}">{{ $estimate->customer->email }}</div>
                        @endif
                    </div>
                    @if(!empty($estimate->customer->phone_number))
                    <a href="tel:{{ $estimate->customer->phone_number }}" class="text-muted flex-shrink-0" style="font-size:.8rem;" title="{{ $estimate->customer->phone_number }}">
                ...
                {{-- Key-value grid --}}
                <div class="ctx-grid">
                    <span class="ctx-k"><i class="bi bi-hash me-1 opacity-50"></i>{{ __('Estimate') }}</span>
                    <span class="ctx-v">#{{ $estimate->id }}</span>

                    @if($estimate->case_number)
                    <span class="ctx-k"><i class="bi bi-file-earmark me-1 opacity-50"></i>{{ __('Case') }}</span>
                    <span class="ctx-v">{{ $estimate->case_number }}</span>
                    @endif
                    ...
                    <span class="ctx-v">
                        <span class="badge rounded-pill text-bg-{{ $statusColor }} fw-normal" style="font-size:.68rem;">
                            {{ ucfirst($estimate->status ?? __('Unknown')) }}
                        </span>
                    </span>

                    <span class="ctx-k"><i class="bi bi-calendar3 me-1 opacity-50"></i>{{ __('Created') }}</span>
                    <span class="ctx-v">{{ $estimate->created_at->format('M d, Y') }}</span>

                    @if($estimate->devices && $estimate->devices->count())
                    <span class="ctx-k"><i class="bi bi-phone me-1 opacity-50"></i>{{ __('Devices') }}</span>
                    <span class="ctx-v d-flex flex-wrap gap-1">
                        @foreach($estimate->devices as $ed)
                        @php $dev = $ed->customerDevice->device ?? null; @endphp
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body p-4">
                <form action="{{ route('tenant.estimates.signatures.store', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}" method="POST">
                    @csrf
                    <input type="hidden" name="signature_type" id="type-hidden" value="{{ old('signature_type', 'approval') }}">

                    {{-- Signature type --}}
                    <div class="mb-1">
                        <label class="form-label fw-semibold">{{ __('Signature Type') }}</label>

                        <label class="type-row approval-row {{ old('signature_type','approval') === 'approval' ? 'selected' : '' }}"
                               onclick="rowSelect(this, 'approval', '{{ __('Approval Signature') }}')">
                            <input type="radio" name="_type_ui" value="approval" class="d-none" {{ old('signature_type','approval') === 'approval' ? 'checked' : '' }}>
                            <span class="tr-icon approval-icon"><i class="bi bi-check2-circle"></i></span>
                            <span class="tr-body">
                                <span class="tr-name">{{ __('Approval') }}</span>
                                <span class="tr-note">{{ __('Customer approves the estimate before work begins.') }}</span>
                            </span>
                        ...
                        <label class="type-row pickup-row {{ old('signature_type') === 'pickup' ? 'selected' : '' }}"
                            ...
                            <input type="radio" name="_type_ui" value="pickup" class="d-none" {{ old('signature_type') === 'pickup' ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                               ...
                               placeholder="{{ __("e.g. Approval Signature — John's iPhone 14") }}"
                               value="{{ old('signature_label', 'Approval Signature') }}"
                               maxlength="255">
                    ...
                    <div class="email-strip d-flex align-items-center gap-3 mb-4">
                        ...
                        <div class="flex-grow-1 min-w-0">
                            ...
                            <div class="text-truncate" style="font-size:.74rem;color:#6b7280;">
                                ...
                                <strong>{{ $estimate->customer->email }}</strong>
                            </div>
                        </div>
                    ...
                    <div class="d-flex align-items-center justify-content-between gap-2 pt-3 border-top flex-wrap">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('tenant.estimates.signatures.index', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}" class="btn btn-outline-secondary btn-sm">
                                {{ __('Cancel') }}
                            </a>
...
<style>
    ...
    .approval-icon  { background: rgba(16,185,129,.10);   color: #059669; }
...
document.addEventListener('DOMContentLoaded', function () {
    const autoLabels = {
        approval: '{{ __("Approval Signature") }}',
