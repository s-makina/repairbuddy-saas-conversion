...
@php
    ...
    $typePillMap = [
        'approval'  => ['pill' => 'wcrb-pill--success', 'label' => __('Approval')],
    ...
    // Build audit log
    $auditLog = [];
    if ($signatureRequest->created_at) {
        $auditLog[] = [
            'label' => __('Signature request created'),
            'timestamp' => $signatureRequest->created_at,
            'color' => 'info',
            'meta' => $signatureRequest->generator?->name ? __('by') . ' ' . $signatureRequest->generator->name : null,
        ];
    }
    if ($signatureRequest->generated_at && $signatureRequest->generated_at != $signatureRequest->created_at) {
        $auditLog[] = [
            'label' => __('Link generated'),
            'timestamp' => $signatureRequest->generated_at,
            'color' => 'info',
        ];
    }
    if ($sigStatus === 'completed' && $signatureRequest->completed_at) {
        $auditLog[] = [
            'label' => __('Signature submitted'),
            'timestamp' => $signatureRequest->completed_at,
            'color' => 'success',
            'meta' => $signatureRequest->completed_ip ? 'IP: ' . $signatureRequest->completed_ip : null,
        ];
    }
    if ($sigStatus === 'expired') {
        $auditLog[] = [
            'label' => __('Link expired'),
            'timestamp' => $signatureRequest->expires_at,
            'color' => 'danger',
        ];
    }
@endphp
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <div>
            ...
            <p class="text-muted small mb-0">
                {{ __('Estimate') }} #{{ $estimate->case_number ?? $estimate->id }}
                @if($estimate->customer) &mdash; {{ $estimate->customer->name }} @endif
            </p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('tenant.estimates.signatures.index', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}"
               class="btn btn-outline-secondary btn-sm">
                <i class="bi bi-list me-1"></i>{{ __('All Requests') }}
            </a>
            <a href="{{ route('tenant.estimates.show', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}"
               class="btn btn-outline-secondary btn-sm">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body" style="font-size:.85rem;">
                    <div class="d-flex flex-column gap-2">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="fw-medium">{{ $estimate->customer->name }}</span>
                        </div>
                        @if($estimate->customer->email)
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">{{ __('Email') }}</span>
                            <span class="fw-medium text-truncate ms-2" style="max-width:160px;">{{ $estimate->customer->email }}</span>
                        </div>
                        @endif
                        @if($estimate->customer->phone)
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">{{ __('Phone') }}</span>
                            <span class="fw-medium">{{ $estimate->customer->phone }}</span>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white border-bottom py-3">
                    <h6 class="mb-0 fw-bold"><i class="bi bi-file-earmark-text me-2 text-muted"></i>{{ __('Estimate Details') }}</h6>
                </div>
                <div class="card-body" style="font-size:.85rem;">
                    <div class="d-flex flex-column gap-2">
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">{{ __('Estimate #') }}</span>
                            <span class="fw-bold text-primary">{{ $estimate->id }}</span>
                        </div>
                        @if($estimate->case_number)
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">{{ __('Case #') }}</span>
                            <span class="fw-medium">{{ $estimate->case_number }}</span>
                        </div>
                        @endif
                        @php
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge bg-{{ $statusColor }}" style="font-size:.72rem;">
                                {{ ucfirst($estimate->status ?? 'pending') }}
                            </span>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span class="text-muted">{{ __('Created') }}</span>
                            <span class="fw-medium">{{ $estimate->created_at?->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <form method="POST"
                              action="{{ route('tenant.estimates.signatures.send', ['business' => $tenant->slug, 'estimateId' => $estimate->id, 'signatureId' => $signatureRequest->id]) }}"
                        ...
                        <a href="{{ route('tenant.estimates.signatures.generator', ['business' => $tenant->slug, 'estimateId' => $estimate->id, 'signatureId' => $signatureRequest->id]) }}"
                           class="btn btn-outline-secondary btn-sm">
            ...
            <div class="text-muted small mt-3 pt-2 border-top px-1">
                ...
                {{ __('Signature ID') }}: #SIG-E{{ $estimate->id }}-{{ $signatureRequest->id }}
...
<style>
    ...
    .wcrb-pill--success { color: #047857; background: rgba(16,185,129,.15);  border-color: rgba(16,185,129,.30); }
    ...
    [data-bs-theme="dark"] .wcrb-pill--success { background: rgba(16,185,129,.25); }
