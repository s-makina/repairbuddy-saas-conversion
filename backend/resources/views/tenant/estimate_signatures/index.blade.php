...
@php
    ...
    $typeBadgeMap = [
        'approval'  => 'wcrb-pill--success',
        'pickup'    => 'wcrb-pill--info',
        'delivery'  => 'wcrb-pill--warning',
        'custom'    => 'wcrb-pill--muted',
    ];

    $approvalEnabled = !empty($signatureSettings['approval_enabled']);
...
<div class="container-fluid p-3">
    ...
    <div class="d-flex flex-wrap gap-2 mb-3 align-items-center">
        ...
        <span class="badge rounded-pill {{ $approvalEnabled ? 'bg-success bg-opacity-10 text-success' : 'bg-secondary bg-opacity-10 text-secondary' }} border px-2 py-1" style="font-size:.75rem;">
            <i class="bi bi-check2-circle me-1"></i>{{ __('Approval') }} {{ $approvalEnabled ? __('ON') : __('OFF') }}
        </span>
        ...
        <span class="badge rounded-pill {{ $deliveryEnabled ? 'bg-success bg-opacity-10 text-success' : 'bg-secondary bg-opacity-10 text-secondary' }} border px-2 py-1" style="font-size:.75rem;">
            <i class="bi bi-truck me-1"></i>{{ __('Delivery') }} {{ $deliveryEnabled ? __('ON') : __('OFF') }}
        </span>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white border-bottom d-flex flex-wrap justify-content-between align-items-center gap-2 py-3">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('tenant.estimates.signatures.create', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}"
        ...
        <div class="card-body p-0">
            @if($signatureRequests->isEmpty())
                <div class="text-center py-5">
                    ...
                    <p class="text-muted small mb-3">{{ __('Create a signature request to collect a customer signature for this estimate.') }}</p>
                    <a href="{{ route('tenant.estimates.signatures.create', ['business' => $tenant->slug, 'estimateId' => $estimate->id]) }}"
                       class="btn btn-primary btn-sm">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" style="font-size:.875rem;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="ps-3">
                                    <a href="{{ route('tenant.estimates.signatures.show', ['business' => $tenant->slug, 'estimateId' => $estimate->id, 'signatureId' => $sig->id]) }}"
                                ...
                                <td class="text-end pe-3">
                                    <div class="d-flex justify-content-end align-items-center gap-1 flex-nowrap">
                                        ...
                                        @if($sig->isPending() && !$sig->isExpired())
                                            <a href="{{ route('tenant.estimates.signatures.generator', ['business' => $tenant->slug, 'estimateId' => $estimate->id, 'signatureId' => $sig->id]) }}"
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end shadow-sm" style="font-size:.82rem;min-width:165px;">
                                                    <li>
                                                        <form method="POST"
                                                              action="{{ route('tenant.estimates.signatures.send', ['business' => $tenant->slug, 'estimateId' => $estimate->id, 'signatureId' => $sig->id]) }}"
...
<style>
    ...
    .wcrb-pill--success { color: #047857; background: rgba(16,185,129,.15);  border: 1px solid rgba(16,185,129,.30); }
