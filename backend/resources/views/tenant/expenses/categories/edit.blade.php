@extends('tenant.layouts.myaccount', ['title' => $pageTitle ?? __('Edit Expense Category')])
			...
			<div class="col-12 col-lg-8">
				<div class="card">
					<div class="card-header">
						<h5 class="card-title mb-0">{{ __('Edit Expense Category') }}</h5>
					</div>
					<div class="card-body">
						<form method="post" action="{{ route('tenant.expense_categories.update', ['business' => $tenant->slug, 'category' => $category->id]) }}">
							@csrf
							@method('POST')
							<div class="row g-3">
								<div class="col-12">
										...
										<div class="col-sm-9">
											<div class="d-flex align-items-center gap-2">
												<input
													...
													value="{{ old('color_code', $category->color_code ?? '#3498db') }}"
										...
										<div class="col-sm-9">
											<select name="parent_category_id" id="parent_category_id" class="form-select @error('parent_category_id') is-invalid @enderror">
												@foreach ($parentOptions as $value => $label)
													<option value="{{ $value }}" {{ old('parent_category_id', $category->parent_category_id) == $value ? 'selected' : '' }}>{{ $label }}</option>
												@endforeach
											</select>
										...
										<div class="col-sm-9 offset-sm-3">
											...
											<div class="form-check form-check-inline">
												<input
													...
													id="taxable"
													value="1"
													{{ old('taxable', $category->taxable ? '1' : '0') ? 'checked' : '' }}
												>
										...
										<div class="col-sm-9">
											<div class="input-group" style="max-width: 150px;">
												<input
													...
													value="{{ old('tax_rate', $category->tax_rate ?? 0) }}"
													min="0"
													max="100"
													step="0.01"
												>
