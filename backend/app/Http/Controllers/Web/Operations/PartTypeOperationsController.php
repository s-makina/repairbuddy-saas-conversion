...
class PartTypeOperationsController extends Controller
{
    public function index(Request $request, string $business)
    {
        ...
        return view('tenant.operations.part-types.index', [
            'tenant' => $tenant,
            'user' => $request->user(),
            'activeNav' => 'operations',
            'pageTitle' => __('Part Types'),
        ]);
    }
    ...
    public function datatable(Request $request, string $business)
    {
        ...
        return DataTables::eloquent($query)
            ...
            ->addColumn('actions_display', function (RepairBuddyPartType $type) use ($tenant) {
                $editUrl = route('tenant.operations.part_types.edit', ['business' => $tenant->slug, 'type' => $type->id]);
                $activeUrl = route('tenant.operations.part_types.active', ['business' => $tenant->slug, 'type' => $type->id]);
                $deleteUrl = route('tenant.operations.part_types.delete', ['business' => $tenant->slug, 'type' => $type->id]);
                $csrf = csrf_field();
    ...
    public function create(Request $request, string $business)
    {
        ...
        $recentTypes = RepairBuddyPartType::query()
            ->orderByDesc('id')
            ->limit(8)
            ->get();

        $parentOptions = RepairBuddyPartType::query()
            ->orderBy('name')
            ->limit(500)
            ->get(['id', 'name'])
        ...
        return view('tenant.operations.part-types.create', [
            'tenant' => $tenant,
            'user' => $request->user(),
            'activeNav' => 'operations',
            'pageTitle' => __('Add Part Type'),
    ...
    public function edit(Request $request, string $business, int $type)
    {
        ...
        $model = RepairBuddyPartType::query()->whereKey($type)->firstOrFail();
        ...
        return view('tenant.operations.part-types.edit', [
            'tenant' => $tenant,
            'user' => $request->user(),
            'activeNav' => 'operations',
            'pageTitle' => __('Edit Part Type'),
            'type' => $model,
            'parentOptions' => $parentOptions,
        ]);
    }
    ...
    public function store(Request $request, string $business): RedirectResponse
    {
        ...
        if ($parentId !== null && ! RepairBuddyPartType::query()->whereKey($parentId)->exists()) {
            return redirect()
                ->route('tenant.operations.part_types.create', ['business' => $tenant->slug])
                ->withErrors(['parent_id' => __('Parent part type is invalid.')])
                ->withInput();
        }

        if (RepairBuddyPartType::query()->where('name', $name)->exists()) {
            return redirect()
                ->route('tenant.operations.part_types.index', ['business' => $tenant->slug])
                ->withErrors(['name' => __('Part type already exists.')])
                ->withInput();
        }

        $typeModel = RepairBuddyPartType::query()->create([
            'name' => $name,
        ...
        if ($request->hasFile('image') && $request->file('image') !== null) {
            ...
            $path = $file->storePublicly('rb-part-types/'.$typeModel->id, ['disk' => 'public']);
            $typeModel->forceFill([
                'image_path' => $path,
            ])->save();
        }
        ...
        return redirect()
            ->route('tenant.operations.part_types.index', ['business' => $tenant->slug])
            ->with('status', __('Part type added.'));
    }
    ...
    public function update(Request $request, string $business, int $type): RedirectResponse
    {
        ...
        if ($parentId !== null && $parentId === (int) $model->id) {
            return redirect()
                ->route('tenant.operations.part_types.edit', ['business' => $tenant->slug, 'type' => $model->id])
                ->withErrors(['parent_id' => __('A part type cannot be its own parent.')])
                ->withInput();
        }
        ...
        if ($request->hasFile('image') && $request->file('image') !== null) {
            ...
            $file = $request->file('image');
            $path = $file->storePublicly('rb-part-types/'.$model->id, ['disk' => 'public']);
            $model->forceFill([
                'image_path' => $path,
            ])->save();
        }
        ...
        return redirect()
            ->route('tenant.operations.part_types.index', ['business' => $tenant->slug])
            ->with('status', __('Part type updated.'));
    }
    ...
    public function delete(Request $request, string $business, int $type): RedirectResponse
    {
        ...
        if ($model->parts()->exists()) {
            return redirect()
                ->route('tenant.operations.part_types.index', ['business' => $tenant->slug])
                ->withErrors(['type' => __('Part type is in use and cannot be deleted.')]);
        }
        ...
        return redirect()
            ->route('tenant.operations.part_types.index', ['business' => $tenant->slug])
            ->with('status', __('Part type deleted.'));
    }
}
