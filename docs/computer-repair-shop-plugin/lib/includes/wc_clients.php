...
defined( 'ABSPATH' ) || exit;
	...
	function wc_comp_rep_shop_clients() {
		if ( ! current_user_can( 'delete_posts' ) || ! current_user_can( 'delete_posts' ) ) {
		...
		if ( isset( $_GET['wcrb_search_clients'] ) && ! empty( $_GET['wcrb_search_clients'] ) ) {
			$search_term = sanitize_text_field( $_GET['wcrb_search_clients'] );
...
?>		
		<div class="wrap" id="poststuff">
				...
				<br class="clear" />
				<form action="admin.php?page=wc-computer-rep-shop-clients" method="get">
					<p class="search-box">
						<label class="screen-reader-text" for="wcrb_search_clients"><?php echo esc_html__( 'Search Clients', 'computer-repair-shop' ); ?>:</label>
						<input type="search" id="wcrb_search_clients" name="wcrb_search_clients" placeholder="<?php echo esc_html( 'By name, email, phone, company, tax id ...', 'computer-repair-shop' ); ?>" value="<?php echo esc_html( $search_term ); ?>" />
						<input type="hidden" name="page" value="wc-computer-rep-shop-clients" />
						<input type="submit" id="client-search-submit" class="button" value="<?php echo esc_html__( 'Search', 'computer-repair-shop' ); ?>">
					</p>
				</form>	
				...
				<tbody data-wp-lists="list:user">
					...
					<?php 
						...
						foreach ( $users_obj->get_results() as $userdata ) {
							...
							$content .= wc_return_jobs_by_user( $userdata->ID, "customer", array() );
							$content .= '</td></tr>';
						}
