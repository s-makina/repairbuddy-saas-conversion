...
export default function EditPartPage() {
  ...
  const params = useParams() as { tenant?: string; business?: string; partId?: string };
  const tenant = params.business ?? params.tenant;
  const partIdParam = params.partId;

  const partId = typeof partIdParam === "string" && partIdParam.length > 0 ? Number(partIdParam) : null;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!partId || !Number.isFinite(partId) || partId <= 0) return;

      setLoading(true);
      setError(null);

      try {
        const [partRes, typesRes, brandsRes, taxesRes] = await Promise.all([
          apiFetch<{ part: ApiPart }>(`/api/${tenant}/app/repairbuddy/parts/${partId}`),
        ...
        setName(p?.name ?? "");
        setSku(p?.sku ?? "");
        setTypeId(typeof p?.part_type_id === "number" ? p.part_type_id : null);
        setBrandId(typeof p?.part_brand_id === "number" ? p.part_brand_id : null);
        setManufacturingCode(p?.manufacturing_code ?? "");
        setStockCode(p?.stock_code ?? "");

        setPrice(p?.price ? (p.price.amount_cents / 100).toFixed(2) : "");
        setCurrency(p?.price?.currency ?? "EUR");
        setTaxId(typeof p?.tax_id === "number" ? p.tax_id : null);

        setWarranty(p?.warranty ?? "");
        setCapacity(p?.capacity ?? "");
        setCoreFeatures(p?.core_features ?? "");

        setInstallationCharges(p?.installation_charges ? (p.installation_charges.amount_cents / 100).toFixed(2) : "");
        setInstallationCurrency(p?.installation_charges?.currency ?? "EUR");
        setInstallationMessage(p?.installation_message ?? "");
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await apiFetch(`/api/${tenant}/app/repairbuddy/parts/${partId}`, {
        method: "PATCH",
        body: payload,
      });

      notify.success("Part updated.");
  ...
  const invalidPartId = !partId || !Number.isFinite(partId) || partId <= 0;
  ...
  return (
    <RequireAuth requiredPermission="parts.manage">
      <div className="space-y-6">
        <PageHeader
          title={invalidPartId ? "Edit part" : `Edit part #${partId}`}
          ...
          actions={
            <>
              ...
              <Button variant="primary" size="sm" type="submit" form="rb_part_edit_form" disabled={disabled || invalidPartId}>
                {busy ? "Saving..." : "Save"}
              </Button>
            </>
          }
        ...
        {invalidPartId ? <div className="text-sm text-red-600">Invalid part id.</div> : null}
        {error ? <div className="text-sm text-red-600">{error}</div> : null}

        <Card className="shadow-none">
          <CardContent className="pt-5">
            <form id="rb_part_edit_form" className="space-y-4" onSubmit={onSubmit}>
              <FormRow label="Name" fieldId="part_name" required>
                <Input id="part_name" value={name} onChange={(e) => setName(e.target.value)} disabled={disabled || invalidPartId} />
              </FormRow>

              <FormRow label="SKU" fieldId="part_sku">
                <Input id="part_sku" value={sku} onChange={(e) => setSku(e.target.value)} disabled={disabled || invalidPartId} />
              </FormRow>
              ...
              <FormRow label="Manufacturing code" fieldId="part_manufacturing_code">
                <Input
                  ...
                  disabled={disabled || invalidPartId}
                />
              </FormRow>
