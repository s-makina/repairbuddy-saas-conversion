...
export default function EditBranchPage() {
  ...
  const params = useParams() as { tenant?: string; business?: string; branch?: string };
  const tenant = params.business ?? params.tenant;
  const branchParam = params.branch;

  const branchId = typeof branchParam === "string" && branchParam.length > 0 ? Number(branchParam) : null;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!branchId || !Number.isFinite(branchId) || branchId <= 0) return;

      setLoading(true);
      setError(null);

      try {
        const res = await apiFetch<{ branch: Branch }>(`/api/${tenant}/app/branches/${branchId}`);
        if (!alive) return;
        ...
        setPhone(b?.phone ?? "");
        setEmail(b?.email ?? "");
        setAddressLine1(b?.address_line1 ?? "");
        setAddressLine2(b?.address_line2 ?? "");
        setAddressCity(b?.address_city ?? "");
        setAddressState(b?.address_state ?? "");
        setAddressPostalCode(b?.address_postal_code ?? "");
        setAddressCountry(b?.address_country ?? "");
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await apiFetch<{ branch: Branch }>(`/api/${tenant}/app/branches/${branchId}`, {
        method: "PUT",
        body: payload,
      });
  ...
  const invalidBranchId = !branchId || !Number.isFinite(branchId) || branchId <= 0;

  return (
    <RequireAuth requiredPermission="branches.manage">
      <div className="space-y-6">
        <PageHeader title="Edit branch" description="Update this location's details." />

        {invalidBranchId ? <div className="text-sm text-red-600">Branch is invalid.</div> : null}
