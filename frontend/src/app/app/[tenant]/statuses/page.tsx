...
export default function TenantStatusesPage() {
  ...
  const openEdit = React.useCallback((domain: Domain, row: { slug: string; label: string; color?: string | null; sort_order?: number | null }) => {
  ...
  const loadAll = React.useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 428 && typeof tenantSlug === "string" && tenantSlug.length > 0) {
        const next = `/app/${tenantSlug}/statuses`;
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              {typeof row.color === "string" && row.color.length > 0 ? (
                <span className="text-xs text-zinc-500">({row.color})</span>
              ) : null}
            </div>
  ...
  return (
    <RequireAuth requiredPermission="settings.manage">
      <ListPageShell
        title="Statuses"
        description="Customize display labels, colors, and ordering for job and payment statuses."
        actions={
        ...
        loading={loading}
        error={error}
        empty={!loading && !error && jobStatuses.length === 0 && paymentStatuses.length === 0}
        emptyTitle="No statuses found"
        emptyDescription="Statuses will appear after seeding and setup."
      >
        {status ? (
        ...
        >
          ...
          <TabsContent value="job">
            <Card className="shadow-none">
              <CardContent className="pt-5">
                <DataTable
                  title={typeof tenantSlug === "string" ? `Job statuses · ${tenantSlug}` : "Job statuses"}
          ...
          <TabsContent value="payment">
            <Card className="shadow-none">
              <CardContent className="pt-5">
                <DataTable
                  title={typeof tenantSlug === "string" ? `Payment statuses · ${tenantSlug}` : "Payment statuses"}
