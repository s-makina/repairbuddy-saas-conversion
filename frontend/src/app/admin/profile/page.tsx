          ...
          <form className="space-y-4" onSubmit={onSave}>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-xs text-zinc-600">
                Changes update your current session after saving.
              </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-[var(--rb-radius-md)] border border-[var(--rb-border)] bg-[var(--rb-surface-muted)] px-4 py-3">
              ...
              <div className="mt-1 text-sm font-semibold text-[var(--rb-text)]">{initialEmail || "—"}</div>
              <div className="mt-1 text-xs text-zinc-600">Status: {emailVerified ? "Verified" : "Not verified"}</div>
            </div>
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            {!emailVerified ? (
              <Button variant="outline" onClick={() => void onResendVerification()} disabled={sendingVerify || isImpersonating || !initialEmail}>
                {sendingVerify ? "Sending..." : "Resend verification email"}
              </Button>
            ) : null}

            <Button variant="outline" onClick={() => void onSendPasswordReset()} disabled={sendingReset || isImpersonating || !initialEmail}>
